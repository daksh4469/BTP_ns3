...
ScheduleVehicleReg()
{
    ...
    for (uint32_t i = 0; i < numVehicle; i++)
    {
        ...
        if (!vehicleApp->reg)
        {
            Simulator::ScheduleWithContext(vehicleApp->GetNode()->GetId(),
                                           Seconds(0),
    ...
    std::cout << "Regs left are " << cnt << std::endl;
    if (cnt)
    {
        std::cout << "Simulated reg again\n";
        Simulator::Schedule(Seconds(10.0), ScheduleVehicleReg);
    }
}
...
ScheduleRSUReg()
{
    Ptr<TAApp> ta = DynamicCast<TAApp>(tas.Get(0)->GetApplication(0));
    ta->systemSetup();
}

void
ScheduleLoginAndAuth()
{
    int cnt = 0;
    for (uint32_t i = 0; i < numVehicle; i++)
    {
        Ptr<VehicleApp> vehicleApp = DynamicCast<VehicleApp>(vehicles.Get(i)->GetApplication(0));
        if (!vehicleApp->loggedIn && vehicleApp->reg)
        {
    ...
    std::cout << "Login and auth left are : " << cnt << std::endl;
    if (cnt)
    {
        std::cout << "Simulated auth again\n";
...
main(int argc, char* argv[])
{
    ...
    AodvHelper aodv;
    // aodv.Set("MaxQueueTime",TimeValue(Seconds(6)));
    // aodv.Set("MaxQueueLen",UintegerValue(128));
    ...
    centralApp->SetStopTime(Seconds(160.0));
    ...
    for (uint32_t i = 0; i < numRSU; ++i)
    {
        ...
        rsuApp->SetStopTime(Seconds(160.0));
    }
    ...
    for (uint32_t i = 0; i < numVehicle; ++i)
    {
        ...
        vehicleApp->SetStopTime(Seconds(160.0));
    }

    std::cout << "LOG: Network Setup completed\n";
    std::cout << "Address of TA : " << tas.Get(0)->GetObject<Ipv4>()->GetAddress(1, 0).GetLocal()
              << "\n";
    Simulator::Schedule(Seconds(5.0), ScheduleRSUReg);
    Simulator::Schedule(Seconds(50.0), ScheduleVehicleReg);
    ...
    // anim.EnableIpv4RouteTracking("vanet-routing-data.xml", Seconds(0), Seconds(20),
    // Seconds(0.5));
         ...
         ++iter)
    {
        ...
        // <<iter->second.rxPackets*100/iter->second.txPackets << "%"); NS_LOG_UNCOND("Packet loss
        // ratio =" << (iter->second.txPackets-iter->second.rxPackets)*100/iter->second.txPackets <<
        // "%"); NS_LOG_UNCOND("Delay =" <<iter->second.delaySum); NS_LOG_UNCOND("Jitter ="
        // <<iter->second.jitterSum); NS_LOG_UNCOND("Throughput =" <<iter->second.rxBytes
    ...
    monitor->SerializeToXmlFile("vanetb.xml", true, true);

    // Cleanup
    Simulator::Destroy();
    std::cout << "Simulation Ended...\n";
    std::cout << "Vehicle to TA Registration :" << cnt1 << std::endl;
    std::cout << "Vehicle recv on TA registration" << cnt2 << std::endl;
    std::cout << "Avg login time: " << tim / numVehicle << "\n";
    std::cout << cnt3 << std::endl;
    std::cout << cnt4 << std::endl;
    std::cout << cnt5 << std::endl;
    std::cout << cnt6 << std::endl;
    // std::cout<<cnt7<<std::endl;
    std::cout << cnt8 << std::endl;
    std::cout << cnt9 << std::endl;
    std::cout << cnt10 << std::endl;
    std::cout << cnt11 << std::endl;
